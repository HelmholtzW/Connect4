...
class DQVAAgent:
    def __init__(self, learning_rate, minibatch_size, replay_memory_size, min_replay_memory_size, update_rate):
        ...
        self.a_model = self.create_q_model()

        # Target model this is what we .predict against every step
        self.target_v_model = self.create_v_model()
        self.target_v_model.set_weights(self.v_model.get_weights())

        # self.target_a_model = self.create_q_model()
        # self.target_a_model.set_weights(self.a_model.get_weights())
    ...
    def train(self, terminal_state):
        ...
        current_as_list = self.a_model.predict(current_states)
        ...
        for index, (current_state, action, reward, new_current_state, done) in enumerate(minibatch):
            ...
            if done:
                ...
                y_t = reward + DISCOUNT * float(future_vs_list[index])

            current_as = current_as_list[index]
            current_as[action] = current_qs_list[index][action] - \
                current_vs_list[index]
        ...
        self.v_model.fit(np.array(X), np.array(vy), batch_size=self.minibatch_size, verbose=0,
                         shuffle=False)
        self.a_model.fit(np.array(X), np.array(ay), batch_size=self.minibatch_size, verbose=0,
                         shuffle=False)
        ...
        if self.target_update_counter > self.update_rate:
            ...
            # a_model_weights = np.array(self.a_model.get_weights())
            # a_target_weights = np.array(self.target_a_model.get_weights())

            # self.target_a_model.set_weights(
            #     a_model_weights * SOFT_UPDATE + a_target_weights * (1 - SOFT_UPDATE))

            self.target_update_counter = 0

    # Queries main network for Q values given current observation space (environment state)
    def get_qs(self, state):
        return self.a_model.predict(np.array(state).reshape(-1, *state.shape))[0]
